...
import { DataSyncStatus, ProductLine, ShippingIndexSnapshot, ShippingOption } from '@/core/store/types';
...
export function Shipping() {
  ...
  const [departureMonth, setDepartureMonth] = useState(new Date().getMonth() + 1);
  ...
  const refreshShippingSnapshot = useCallback(async (): Promise<ShippingOption[]> => {
    ...
    try {
      ...
      try { setShippingIndices(await dataProvider.getShippingIndexSnapshot()); } catch { setShippingIndices(null); }
  ...
  useGSAP(() => {
    const cards = intelligenceRef.current?.querySelectorAll('.transport-decision-card');
    if (!cards?.length) return;
    const mm = gsap.matchMedia();
    mm.add('(prefers-reduced-motion: no-preference)', () => { gsap.fromTo(cards, { autoAlpha: 0, y: 8 }, { autoAlpha: 1, y: 0, duration: .3, stagger: .04, ease: 'power2.out', clearProps: 'transform' }); });
    return () => mm.revert();
  }, { scope: intelligenceRef, dependencies: [routes.length, departureMonth, orderInput.productLine], revertOnUpdate: true });
  ...
  return <div className="shipping-assistance">
    ...
    {transportDecisions.length > 0 && <section ref={intelligenceRef} className="transport-intelligence" aria-label="运输决策建议"><div className="section-heading"><div><span className="section-index">02</span><div><h2>运输决策建议</h2><p>基于产品、货量、起运月份、路线样本与航运指数的规则推演，不替代船公司承诺。</p></div></div><span className="data-rule">运费仅输出估算区间</span></div><div className="transport-decision-grid">{transportDecisions.map(({ route, decision }) => <article key={route.option_id} className="transport-decision-card"><div className="transport-decision-head"><div><span className="section-eyebrow">{portLabel(route.port_origin)} → {portLabel(route.destination_port)}</span><h3>推荐{decision.vessel}</h3></div><span className={`transport-risk transport-risk-${decision.risk === '高' ? 'high' : decision.risk === '中' ? 'medium' : 'low'}`}>{decision.risk}风险</span></div><div className="transport-kpis"><div><span>运费估算</span><strong>{decision.freightMin == null ? '数据不足' : `${decision.freightMin}—${decision.freightMax} USD/吨`}</strong></div><div><span>ETA</span><strong>{route.eta}</strong></div><div><span>指数参考</span><strong>{decision.indexLabel}</strong></div></div><p className="transport-reason">{decision.reasons[0]}；{decision.reasons[1]}。</p><button className="decision-expand" aria-expanded={expandedRoute === route.option_id} onClick={() => setExpandedRoute(expandedRoute === route.option_id ? null : route.option_id)}>{expandedRoute === route.option_id ? '收起判断依据' : '展开判断依据'}</button>{expandedRoute === route.option_id && <div className="transport-detail"><p><b>钢卷防护</b>：{decision.protection.join('；')}</p><p><b>季节条件</b>：{decision.seasonal}</p><p><b>时效与合同</b>：{decision.timing}</p><p><b>航线宏观</b>：{decision.macro}</p><p><b>风险依据</b>：{decision.reasons.join('；')}</p><p className="transport-gaps">数据边界：{decision.gaps.join('；')}</p></div>}</article>)}</div></section>}
  </div>;
}
